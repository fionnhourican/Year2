/*
 ...
 Date: 30/11/22
 requirements: Array of int.
 description: Prints most frequent. 
...
void sortNumbers(int *pNumbers, int length);
...
Description:
    ...
    - checks if count is higher than previous max
    - prints max after loop.
...
void sortNumbers(int *pNumbers, int length) {
    for (int i = 0; i < length; i++ )
    {
        ...
        {
            if (pNumbers[j] > pNumbers[i]) // Use bubble sort
            {
                int tmp = pNumbers[i];
